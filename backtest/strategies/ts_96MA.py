...
def analyze_stock_96mv(ts_code: str, df: pd.DataFrame,
    ...
    try:
        ...
        # ─── HARD FILTER 3: MA96 slope must be flat or rising (≥ -0.5%) ───
        if ma96_slope < MA96_MIN_SLOPE:
            return None

        # ─── MA60 slope (scoring only, not a hard filter) ───
        ...
        # ─── Bounce calculation (scoring only, not a hard filter) ───
        ...
        # 5b. MA60 Slope bonus (5 pts): both timeframes rising = resonance
        if ma60_slope is not None and not pd.isna(ma60_slope):
            if ma60_slope > 0.5:
                score += 5.0
                signals.append(f"MA60_slope_up({ma60_slope:.2f}%)")
            elif ma60_slope > 0.0:
                score += 3.0
                signals.append(f"MA60_slope_flat_up({ma60_slope:.2f}%)")
...
def pick_96mv_stocks(end_date: str, max_picks: int = 10) -> pd.DataFrame:
    ...
    # Always stay invested — ts_7AZ has 0 zero-pick days and +70.6%.
    # Circuit breakers and exposure scaling caused 29 zero-pick days
    # and -9.11% (worse than the original -4.60% with no filters).
    # Only skip in explicit bear regime (price below MA60+MA120).
    if regime == 'bear':
        logger.warning("[ts_96MA] Bear regime — 0 picks (capital protection).")
        return pd.DataFrame()

    max_picks = 12
    min_score = 40

    logger.info(f"[ts_96MA] max_picks={max_picks}, min_score={min_score}")
