#!/usr/bin/env bash
# Compress videos >= 500MB in ~/Downloads/jav/ to mobile-friendly format
# Uses h264 CRF 27, 720p max, AAC 128k, faststart
# Saves compressed version as .compressed.mp4 then replaces original
...
# Determine min bytes for find (500 * 1024 * 1024)
MIN_BYTES=$((MIN_SIZE_MB * 1024 * 1024))

# Log header only if starting fresh (detectable by missing state file or empty)
...
# Read already-done files from state file
declare -A DONE_SET
if [ -f "$STATE_FILE" ]; then
...
# Find files >= MIN_SIZE_MB, sort by size descending
...
for FILE in "${FILES[@]}"; do
    ...
    # Probe original video bitrate (kbps)
    ...
    ORIG_BITRATE=$((ORIG_BITRATE / 1000))  # kbps
    
    # For mobile: always compress files > 500MB, target ~300MB max
    # Calculate target bitrate based on duration to hit ~300MB
    DURATION=$(ffprobe -v error -show_entries format=duration -of default=nokey=1:noprint_wrappers=1 "$FILE" 2>/dev/null || echo "0")
    DURATION=${DURATION%.*}
    if [ "$DURATION" -gt 0 ]; then
        # Target ~300MB = 300 * 8 * 1024 = 2457600 kbits total
        # Subtract audio (~128kbps * duration)
    ...
    if [ "$WIDTH" -gt 1280 ] 2>/dev/null; then
        ...
        SCALE_FILTER="scale='iw':-2"  # keep original width for <= 852p
    fi

    START_TIME=$(date +%s)

    # Run ffmpeg compression with target bitrate
    if ffmpeg -y -i "$FILE" \
        -c:v libx264 \
