...
def fill_stock_code(code: str, ui_text: str) -> None:
    ...
    4. The app auto-returns to the buy order page.
    """
    ...
    if stock_center:
        ...
        device_tap(*stock_center, sleep_after=3)  # Wait for auto-return to buy page
    else:
        logger.error(f"Stock {code} not found in search results")
        raise RuntimeError(f"Stock {code} not found in search results")

    # Step 5: Verify we're back on the buy page
    buy_ui = get_ui_tree()
    if not verify_on_buy_page(buy_ui):
        logger.warning("May not have returned to buy page after stock selection")
    else:
        logger.info("✅ Stock selected, back on buy order page")
...
def fill_quantity(quantity: str, ui_text: str) -> None:
    ...
    Finds the EditText after "买入数量" (or "买入数量") label.
...
def create_buy_order(code: str, price: str, quantity: str, submit: bool = False, dry_run: bool = False) -> None:
    ...
    """
    logger.info(f"Creating buy order: code={code}, price={price}, quantity={quantity}, submit={submit}")

    if dry_run:
        logger.info("[DRY RUN] Would fill: stock={}, price={}, qty={}".format(code, price, quantity))
        return

    # Step 0: Navigate to homepage, login, then open the 到价买入 page
    goto_page('order_buy')
    ...
    # Step 1: Read current UI state and verify buy page
    ui_text = get_ui_tree()
  ...
  python trading/create_order_buy.py --show-ui
    ...
    if args.json:
        try:
            ...
            if stock_data:
                # Use price for buy order
                if 'buy_price' in stock_data:
                    args.price = str(stock_data['buy_price'])
