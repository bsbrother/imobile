...
def fill_quantity(quantity: str, ui_text: str) -> None:
    ...
    Finds the EditText after "卖出数量" label.
...
def create_sell_order(code: str, price: str, quantity: str, submit: bool = False, dry_run: bool = False) -> None:
    ...
    """
    logger.info(f"Creating sell order: code={code}, price={price}, quantity={quantity}, submit={submit}, dry_run={dry_run}")
    ...
    time.sleep(2)
    ui_text = get_ui_tree()
    center = find_element_center(ui_text, '到价卖出')
    if not center:
    ...
    if submit and not dry_run:
        ...
        logger.info(f"[DRY RUN] Would submit sell order: code={code}, price={price}, quantity={quantity}")
    else:
    ...
    if args.json:
        try:
            ...
            for order in data.get('smart_orders', []):
                if 'sell_take_profit_price' in order and 'buy_quantity' in order:
                    ...
                    price = str(order['sell_take_profit_price'])
        ...
        if args.price and args.quantity:
            ...
            try:
                ...
                if not orders_to_process:
                    logger.warning("No sell orders generated in the JSON file.")
            except Exception as e:
