            ...
            while i < len(lines):
                ...
                if 'TextView' in line and ('到价' in line or '止盈' in line or '止损' in line):
                    ...
                    if order_number:
                        ...
                        # Extract the actual event date from the order number for cutoff comparison
                        card_date = None
                        m_num = re.search(r'^(20\d{6})', str(order_number))
                        if m_num:
                            ds = m_num.group(1)
                            card_date = f"{ds[:4]}-{ds[4:6]}-{ds[6:8]}"
                            
                        if not card_date:
                            for cline in card_lines:
                                m = re.search(r'((\d{4}[-/])?\d{2}[-/]\d{2}( \d{2}:\d{2}:\d{2})?)', cline)
                                if m:
                                    found_date = m.group(1).replace('/', '-')
                                    # Ignore if this date is exactly the valid_until date
                                    if valid_until and found_date in valid_until.replace('/', '-'):
                                        continue
                                    
                                    if len(found_date) >= 14 and ":" in found_date and found_date.count('-') == 1:
                                        # e.g. "06-30 14:00:00" -> prepend year
                                        found_date = f"{current_year}-{found_date}"
                                    elif len(found_date) == 5: # MM-DD
                                        found_date = f"{current_year}-{found_date}"
                                        
                                    if len(found_date) >= 10:
                                        card_date = found_date[:10]
                                        break
                                    
                        if not card_date and reason_of_ending:
                            m = re.search(r'(\d{4}[-/]\d{2}[-/]\d{2})', reason_of_ending)
                            if m:
                                card_date = m.group(1).replace('/', '-')

                        if stop_before_date and card_date:
                            if card_date < stop_before_date:
                                logger.info(f"Skipping order {order_number} with event date {card_date} before cutoff {stop_before_date}")
                                skip_order = True
                                if tab_name == "已结束":
                                    logger.info("Stopping scroll for '已结束' tab.")
                                    stop_tab_scrolling = True
                        
                        # Safety net for very old valid_until
                        if not skip_order and valid_until:
                            m_year = re.search(r'(\d{4})', valid_until)
                            if m_year and int(m_year.group(1)) < current_year:
                                logger.info(f"Skipping order {order_number} with validity before current year: {valid_until}")
                                skip_order = True
                                if tab_name == "已结束":
                                    logger.info("Stopping scroll for '已结束' tab (valid_until safety).")
                                    stop_tab_scrolling = True
    ...
    for scroll_idx in range(40):  # Scroll max 40 times
        ...
        for y1, y2 in rows:
                ...
                if m_bounds:
                    ...
                    if y1 <= cy <= y2:
                        ...
                        if match_txt:
                            ...
                            # Check for a transaction/order number starting with date
                            m_id = re.search(r'^(20\d{6})\d+', text)
                            if m_id:
                                ds = m_id.group(1)
                                tx_id_date = f"{ds[:4]}-{ds[4:6]}-{ds[6:8]}"
            ...
            if name and time_str:
                current_year = datetime.now().year
                if len(time_str) >= 19:  # e.g., "2026-04-14 10:35:56" or "2026/04/14 10:35:56"
                    tx_date = time_str.replace('/', '-')
                elif len(time_str) == 14:  # "06/23 10:35:56"
                    tx_date = f"{current_year}-{time_str[0:2]}-{time_str[3:5]} {time_str[6:]}"
                else:
                    tx_date = time_str
                    
                if tx_id_date:
                    tx_date = tx_id_date + (tx_date[10:] if len(tx_date) > 10 else " 00:00:00")
...
def get_default_cutoff_date() -> str | None:
    """Get the previous trading date as the default cutoff date for incremental sync."""
    try:
        from backtest.utils.trading_calendar import get_trading_days_before
        from datetime import datetime
        today = datetime.now().strftime('%Y%m%d')
        prev = get_trading_days_before(today, 1)
        if prev and isinstance(prev, str) and len(prev) == 8:
            return f"{prev[:4]}-{prev[4:6]}-{prev[6:]}"
        elif isinstance(prev, list) and prev:
            p = prev[0]
            return f"{p[:4]}-{p[4:6]}-{p[6:]}"
    except Exception as e:
        logger.warning(f"Failed to get previous trading date, falling back to .env: {e}")
    return os.getenv('START_REAL_TRADING_DATE', '2026-06-29')

# Incremental sync cutoff: only sync data on or after this date.
# Default is the previous trading date.
# Can be overridden by set_sync_cutoff_date() (e.g. for pre-start init).
_sync_cutoff_date: str | None = get_default_cutoff_date()
...
def set_sync_cutoff_date(date_str: str | None):
    ...
    Call without arguments to reset to the default (previous trading date).
    """
    global _sync_cutoff_date
    if date_str is None:
        _sync_cutoff_date = get_default_cutoff_date()
    else:
...
async def check_app_vs_db(user_id: int = 1) -> dict:
    ...
    app_history_orders_count = 0
    # 2. Running & History orders → smart_orders count
    try:
        order_csv = await get_order_from_app_smart_order_page_structured(
            config, llm, tools, target_tabs=["运行中", "今日已触发", "已结束"],
            stop_before_date=_sync_cutoff_date
        )
        if order_csv:
            _, ord_rows = parse_csv_data(order_csv)
            app_running_orders_count = sum(1 for r in ord_rows if r[8] == '运行中')
            app_history_orders_count = sum(1 for r in ord_rows if r[8] != '运行中')
    ...
    try:
        tx_csv = await get_transactions_from_app_history_page_structured(
            config, llm, tools, stop_before_date=_sync_cutoff_date
        )
        if tx_csv:
    ...
    result['app_state']['history_orders'] = app_history_orders_count
    ...
    with DB.cursor() as cursor:
        ...
        db_running = db_tx_count = db_history_orders = 0
        current_year = datetime.now().year
        # 3. smart_orders — running orders count
        db_running = cursor.execute(
            "SELECT COUNT(*) FROM smart_orders WHERE status='running' AND user_id=? AND order_number NOT LIKE 'ORD_%'",
            (user_id,)
        ).fetchone()[0]
        
        order_boundary_str = _sync_cutoff_date.replace('-', '') if _sync_cutoff_date else f"{current_year}0101"
        db_history_orders = cursor.execute(
            "SELECT COUNT(*) FROM smart_orders WHERE status!='running' AND user_id=? AND order_number >= ? AND order_number NOT LIKE 'ORD_%'",
            (user_id, order_boundary_str)
        ).fetchone()[0]
        ...
        result['db_state']['history_orders'] = db_history_orders
        ...
        if app_history_orders_count > 0 and db_history_orders != app_history_orders_count:
            result['db_matches_app'] = False
            result['mismatches'].append(
                f'smart_orders(history): DB={db_history_orders} vs App={app_history_orders_count}'
            )
...
async def cron_sync_app_to_db(check_trading_day_and_time: bool = True) -> dict:
    ...
    # Fetch all relevant tabs for incremental sync, using _sync_cutoff_date
    order_csv = await get_order_from_app_smart_order_page_structured(
    ...
    if _has_orders:
        result_order = sync_order_data_to_db(order_csv, user_id=1, cutoff_date=_sync_cutoff_date)
